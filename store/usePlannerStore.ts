...
type Store = { tasks:Task[]; goals:Goal[]; checkIn:CheckIn; coins:number; lifeProgress:number; season:Season; furniture:FurnitureItem[]; rewards:Reward[]; setCheckIn:(v:Partial<CheckIn>)=>void; setSeason:(s:Season)=>void; updateTask:(id:string,status:TaskStatus)=>void; replan:(hours:number)=>void; addParsedTasks:(tasks:Task[])=>void; buyItem:(id:string)=>void; moveItem:(id:string,x:number,y:number)=>void; addReward:(title:string,cost:number)=>void; deleteReward:(id:string)=>void };
export const usePlannerStore = create<Store>()(persist((set)=>({
 tasks:initialTasks,goals:initialGoals,checkIn:{wakeUp:'12:40',energy:3,focusedHours:4,fixedPlan:'17:00 Research meeting'},coins:460,lifeProgress:38,season:'Autumn',furniture:initialFurniture,rewards:initialRewards,
 ...
 addReward:(title,cost)=>set(s=>({rewards:[...s.rewards,{id:Date.now().toString(),title,cost}]})),deleteReward:(id)=>set(s=>({rewards:s.rewards.filter(r=>r.id!==id)})),
